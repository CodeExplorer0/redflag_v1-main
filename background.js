// ClearHire Background Script
// Manages message routing, storage, and extension state

console.log("ClearHire: Background script loaded");

// Main message handler for content script and popup
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log("ClearHire: Background received message:", message);
  ...
  switch (message.type) {
    ...
    case "GET_REPORTS":
      ...
      return true; // Async response
    default:
      console.warn("ClearHire: Unknown message type:", message.type);
...
function storeAnalysisResult(data, sender, sendResponse) {
  console.log("ClearHire: Analysis result received:", data);
  ...
  chrome.storage.local.get(["recentAnalyses"], (result) => {
    ...
    chrome.storage.local.set({ recentAnalyses: analyses }, () => {
      if (chrome.runtime.lastError) {
        console.error(
          "ClearHire: Error storing analysis result:",
...
function storeUserReport(data, sendResponse) {
  console.log("ClearHire: User report received:", data);
  ...
  chrome.storage.local.get(["userReports"], (result) => {
    ...
    chrome.storage.local.set({ userReports: reports }, () => {
      if (chrome.runtime.lastError) {
        console.error(
          "ClearHire: Error storing user report:",
          chrome.runtime.lastError
        );
        sendResponse({
          success: false,
        ...
        console.log("ClearHire: Report saved locally");
        sendResponse({ success: true });
      }
    });
  });
...
function fetchSettings(sendResponse) {
  chrome.storage.local.get(
    ...
    (settings) => {
      if (chrome.runtime.lastError) {
        console.error(
          "ClearHire: Error fetching settings:",
          chrome.runtime.lastError
        );
        sendResponse({
          success: false,
...
function persistSettings(settings, sendResponse) {
  chrome.storage.local.set(settings, () => {
    if (chrome.runtime.lastError) {
      ...
      console.log("ClearHire: Settings saved:", settings);
      sendResponse({ success: true });
    }
  });
}
...
function fetchReports(sendResponse) {
  chrome.storage.local.get(["userReports"], (result) => {
    if (chrome.runtime.lastError) {
      console.error(
        "ClearHire: Error fetching reports:",
        chrome.runtime.lastError
      );
      sendResponse({
        success: false,
...
chrome.runtime.onInstalled.addListener((details) => {
  console.log("ClearHire: Extension installed/updated:", details);
  if (details.reason === "install") {
    chrome.storage.local.set(
      ...
      () => {
        if (chrome.runtime.lastError) {
          console.error(
            "ClearHire: Error setting default values on install:",
            chrome.runtime.lastError
          );
        } else {
          console.log("ClearHire: Default settings initialized.");
        }
      }
    );
  }
